...
import { CHAT_MODEL_PRIMARY, CHAT_MODEL_SECONDARY } from './groqModels.js';
...
// Model ids come from the shared registry (lib/groqModels.js) so the pre-run
// availability gate can see them and a retirement can be routed around with a
// variable. CHAT_GROQ_MODEL remains the override for the primary.
export { CHAT_MODEL_PRIMARY };

// Retrying the identical model is not a real fallback, so drop it if the primary
// override happens to name the same id (same rationale as the scraper cascade's
// de-duplication).
export const CHAT_MODEL_FALLBACKS = [CHAT_MODEL_SECONDARY].filter(
  (model) => model !== CHAT_MODEL_PRIMARY,
);
