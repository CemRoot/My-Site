...
export function formatPageContext(context) {
  ...
  if (context.content) {
    ...
    const clipped = content.length > 4500 ? `${content.slice(0, 4500)}…` : content;
    parts.push(`\n📖 Main Content:\n${clipped}`);
  }
  if (context.lastUpdated) parts.push(`\n🕐 Last Updated: ${context.lastUpdated}`);

  parts.push('\n=== END OF PAGE INFORMATION ===');
  parts.push('\nPrioritize this page context for questions about the current article/section. Combine it with site knowledge about Cem when useful.');
