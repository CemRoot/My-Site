 ...
 *
 * Usage:
 *   node scripts/ci/check-groq-models.mjs [--scope=scraper|linkedin|chat|all]
 *
 * Exit codes:
 *   0 — every required tier in scope is available (missing optional tiers warn)
 *   1 — a required tier is missing, a credential is missing, or the model list
 *       could not be read for any non-transient reason (and for a transient one
 *       when STRICT_GROQ_MODEL_CHECK=true)
 *
 * A transient Groq outage must not block the pipeline, so a connection failure
 * or a 5xx warns and exits 0 by default. Nothing else is treated that way: a
 * rejected key, a 4xx, an empty list or an unparseable body all mean the gate
 * cannot answer its question, and are fatal regardless of STRICT.
 *
 * Env:
 ...
 *   GROQ_*_MODEL / CHAT_GROQ_MODEL — tier overrides, see lib/groqModels.js
 */

// scripts/lib/config.js loads .env / .env.local for local runs; the registry
// itself reads process.env directly so it stays importable from Vercel too.
import '../lib/config.js';
import { GROQ_MODEL_TIERS, GROQ_CREDENTIALS, envVarForTier } from '../../lib/groqModels.js';
...
/**
 * Fail-open is only ever correct for something that will heal on its own: the
 * connection never completed, or Groq answered 5xx. Everything else — a rejected
 * key, a 4xx, an empty 2xx list, a body that will not parse — means the gate
 * cannot answer the question it exists to answer, and letting the run proceed
 * would spend Firecrawl credits on a scrape that may not be able to translate.
 * So transience is declared explicitly here rather than inferred from "it threw".
 */
class GroqCheckError extends Error {
...
async function fetchAvailableModels(apiKey) {
  ...
  try {
    ...
    throw new GroqCheckError(`could not reach Groq (${error?.message || error})`, {
      transient: true,
    });
  }

  if (res.status >= 500) {
    throw new GroqCheckError(`Groq /models returned HTTP ${res.status}`, { transient: true });
  }

  if (res.status === 401 || res.status === 403) {
    // Never echo the body — it is provider output going into public CI logs.
    throw new GroqCheckError(`Groq rejected the API key (HTTP ${res.status})`, {
      transient: false,
    });
  }

  if (!res.ok) {
    throw new GroqCheckError(`Groq /models returned HTTP ${res.status}`, { transient: false });
  }
  ...
  try {
    ...
    throw new GroqCheckError('Groq /models returned a body that could not be parsed', {
      transient: false,
    });
  }
  ...
  if (ids.length === 0) {
    throw new GroqCheckError('Groq /models returned an empty model list', { transient: false });
  }
...
function printRemediation(missing) {
  ...
  console.error('\nThen update lib/groqModels.js so the default is');
...
async function main() {
  ...
  if (tiers.length === 0) {
    console.error(`❌ Unknown scope "${SCOPE}" — expected scraper, linkedin, chat or all.`);
    process.exit(1);
  }
  ...
  for (const name of neededCredentials) {
    ...
    try {
      ...
      // Only an explicitly transient failure may fail open.
      if (error?.transient !== true) {
        console.error(`❌ ${credential.envVar}: ${message}`);
        console.error('   This will not resolve on its own, so the gate cannot confirm the');
        console.error('   models are available. Failing now rather than spending Firecrawl');
        console.error('   credits on a run that may not be able to translate.');
        process.exit(1);
      }
      ...
      console.warn('   Transient failure; continuing, the run cascade will surface real errors.');
      process.exit(0);
    }
  }
