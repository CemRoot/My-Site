 ...
 * 2026-08-16). Two things follow from that, and this module exists to provide
 * both:
 *
 ...
 * a call site is not — that is how the LinkedIn digest kept a dead id after the
 * scraper had been migrated.
 */
...
/**
 * Which credential each tier is called with. The parser runs on its own key
 * (`GROQ_PARSER_API_KEY`, falling back to `GROQ_API_KEY`), and a key only sees
 * the models its own account can access — so a tier must be validated against
 * the credential that will actually call it, not against whichever key is handy.
 */
export const GROQ_CREDENTIALS = {
  default: { envVar: 'GROQ_API_KEY', value: () => env.GROQ_API_KEY },
  parser: { envVar: 'GROQ_PARSER_API_KEY', value: () => env.GROQ_PARSER_API_KEY },
};
...
export const envVarForTier = (tier) => `GROQ_${tier.replace(/-/g, '_').toUpperCase()}_MODEL`;
